...
| 9b | ch05 | TRM 沒有描述分條規則 | 實測解出 **加總 = 特徵圖高 + (片數−1)×(k_h − s_y)**，兩模型 37 層全中 | TRM 沒收錄，**本書解出**（[ch05](./ch05_regcmd.md) §8） |
...
| [ch05](./ch05_regcmd.md) 初稿推測 | 分條多出的 2 列 = 卷積接縫重疊 | YOLOv5s 的加總剛好等於輸入高，**推測被推翻**，改標待查。**後來把卷積參數暫存器也解出來，證實方向對但公式不完整** —— 正解是 `(片數−1)×(k_h − s_y)`（[ch05](./ch05_regcmd.md) §8） |
...
| # | 實驗 | 結果 |
|---|---|---|
| 9.1 | 手動觸發軟重置，比對前後暫存器 | `0x0010` 從上一個 task 的 IOVA `0xffc3fd00` 變回 `0x1`；`0x0030` 回到 TRM 的 reset value `0x0`；`dmesg` 的 `num: 6` 對上 dtsi 的 6 個 reset |
